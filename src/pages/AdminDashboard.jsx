...
//   // Note: 'date' will now hold "YYYY-MM-DDTHH:MM" format
...
//     try {
//       // Ensure date is not empty before sending
//       if (!eventForm.date) {
...
//                     type="datetime-local" 
//                     className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none transition" 
//                   />
//                 </div>
...
//                     <Users size={16} />{event.interested_count} People Interested
//                   </div>
...
const AdminDashboard = () => {
  ...
  const [eventForm, setEventForm] = useState({ title: '', description: '', date: '', end_date: '', location: '' });
  ...
  const fetchEvents = async (currentUser) => {
    try {
      ...
      if (Array.isArray(eventData) && currentUser) {
        const myFilteredEvents = eventData.filter(event => {
            let eventCreatorId = event.created_by || event.created_by_id;
            if (typeof eventCreatorId === 'object' && eventCreatorId !== null) eventCreatorId = eventCreatorId.id;
            return String(eventCreatorId) === String(currentUser.id);
        });
        setEvents(myFilteredEvents);
      } else {
  ...
  const resetForm = () => {
      setEventForm({ title: '', description: '', date: '', end_date: '', location: '' });
      setIsEditing(false);
      setEditEventId(null);
      setShowCreateEvent(false);
  };

  // Helper to format ISO date string to input datetime-local format (YYYY-MM-DDTHH:MM)
  ...
  const handleUpdateEvent = async () => {
      try {
          if (!eventForm.date || !eventForm.end_date) {
            alert("Please select both start and end dates");
            return;
          }
          // Assuming eventsAPI has an update method: update(id, data)
          await eventsAPI.update(editEventId, eventForm);
          resetForm();
          fetchEvents(user);
      } catch (error) {
  ...
  const renderEventManagement = () => (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
        <div className="mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <h2 className="text-3xl font-bold text-gray-800">Manage Events</h2>
          {!showCreateEvent && (
              <button onClick={() => { resetForm(); setShowCreateEvent(true); }} className="w-full md:w-auto flex items-center justify-center gap-2 bg-purple-600 text-white px-6 py-3 rounded-lg hover:bg-purple-700 transition shadow-lg">
                <Plus size={20} /> Create New Event
              </button>
          )}
        </div>
        ...
        {showCreateEvent && (
          <div className="bg-white rounded-lg shadow-xl p-6 mb-6 border-l-4 border-purple-500 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button onClick={resetForm} className="bg-gray-100 text-gray-600 px-6 py-2 rounded-lg hover:bg-gray-200 font-medium transition">Cancel</button>
                <button 
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events?.length > 0 ? (
                ...
                <div key={event.id} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition border-t-4 border-purple-400 flex flex-col h-full">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-1">
                        {/* Edit Button */}
                        <button onClick={() => handleEditClick(event)} className="text-blue-400 hover:text-blue-600 p-2 hover:bg-blue-50 rounded transition" title="Edit Event">
                            <Edit size={20} />
                        </button>
                        {/* Delete Button */}
                        <button onClick={() => deleteEvent(event.id)} className="text-red-400 hover:text-red-600 p-2 hover:bg-red-50 rounded transition" title="Delete Event">
                            <Trash2 size={20} />
                        </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4 bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      <Clock size={16} className="text-purple-600"/>
                      <span className="font-medium">{start.time} - {end.time}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-700">
                      <MapPin size={16} className="text-purple-600"/>
                      <span>{event.location}</span>
            ...
            <div className="col-span-3 text-center py-12 bg-white rounded-lg shadow border-2 border-dashed border-gray-200">
                <Calendar className="mx-auto text-gray-300 mb-2" size={48} />
                <p className="text-gray-500 text-lg">You haven't created any events yet.</p>
                <button onClick={() => { resetForm(); setShowCreateEvent(true); }} className="mt-2 text-purple-600 font-medium hover:underline">Create your first event</button>
            </div>
          )}
        </div>
    </div>
  ...
  if (loading) return <div className="min-h-screen bg-purple-50 flex items-center justify-center"><Loader2 className="animate-spin text-purple-600" size={48} /></div>;
      ...
      <nav className="bg-white shadow-md border-b-4 border-purple-500 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center bg-gray-100 p-1 rounded-lg">
                <button onClick={() => setActiveTab('events')} className={`flex items-center gap-2 px-6 py-2 rounded-md transition-all font-medium ${activeTab === 'events' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500 hover:text-purple-600'}`}>
                    <Calendar size={18} /> Events
                </button>
                <button onClick={() => setActiveTab('analytics')} className={`flex items-center gap-2 px-6 py-2 rounded-md transition-all font-medium ${activeTab === 'analytics' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-500 hover:text-purple-600'}`}>
                    <BarChart3 size={18} /> Analytics
                </button>
            </div>
            <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 text-red-500 hover:bg-red-50 rounded-lg transition font-medium text-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {selectedEvent && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
            <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                  <h3 className="text-xl font-bold mb-4 text-purple-800 flex items-center gap-2"><Users size={20} /> Interested Users</h3>
                  {interestedUsers.length > 0 ? (
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                      {interestedUsers.map((interest) => (
                        <div key={interest.id} className="p-3 bg-purple-50 rounded-lg border-l-4 border-purple-400 hover:bg-purple-100 transition">
                  ...
                  ) : (<div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed border-gray-300"><Users className="mx-auto text-gray-400 mb-2" /><p className="text-gray-500">No users interested yet.</p></div>)}
              </div>
              <div className="flex-1 border-l pl-0 md:pl-6 border-gray-200">
                <h3 className="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2"><Send size={20} className="text-blue-600" /> Send Update</h3>
                <div className="space-y-4">
                    <input type="text" placeholder="Subject" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} />
                    <textarea rows="4" placeholder="Message content..." className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={emailMessage} onChange={(e) => setEmailMessage(e.target.value)} />
                    <div className="flex gap-3 pt-2">
                        <button onClick={() => setSelectedEvent(null)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">Close</button>
                        <button onClick={handleSendNotification} disabled={sendingEmail || interestedUsers.length === 0} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-white font-medium ${sendingEmail || interestedUsers.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                            {sendingEmail ? 'Sending...' : <><Send size={16} /> Send Email</>}
                        </button>
                    </div>
                </div>
