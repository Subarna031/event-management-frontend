...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    setUser(parsedUser);
    // Fetch events immediately using the parsed user data
    fetchEvents(parsedUser);
  }, [navigate]);

  // --- UPDATED FETCH FUNCTION FOR 'created_by_id' ---
    ...
    try {
      ...
      if (Array.isArray(eventData) && currentUser) {
        ...
        const myFilteredEvents = eventData.filter(event => {
            // 1. Get the creator ID from the event.
            // API usually maps DB 'created_by_id' -> 'created_by' in JSON
            // We check both just in case your serializer is different.
            let eventCreatorId = event.created_by || event.created_by_id;

            // 2. Handle case where 'created_by' is an object (e.g. {id: 1, username: "admin"})
            if (typeof eventCreatorId === 'object' && eventCreatorId !== null) {
                eventCreatorId = eventCreatorId.id;
            }

            // 3. Compare with the logged-in user's ID
            // We use String() to ensure '5' equals 5 (type safety)
  ...
  const handleCreateEvent = async () => {
    try {
      ...
      setShowCreateEvent(false);
      setEventForm({ title: '', description: '', date: '', location: '' });
      fetchEvents(user); // Refresh list with current user filter
    } catch (error) {
      console.error('Error creating event:', error);
      alert('Failed to create event');
    }
